...
public class JsonUtils {
    ...
    public static byte[] objectToBytes(Object subject) {
        return objectToBytesOrDefault(subject, null);
    }
    ...
    public static byte[] objectToBytesOrDefault(Object subject, byte[] defaultValue) {
        if (Objects.isNull(subject)) {
            return defaultValue;
        }
        try {
            return OBJECT_WRITER.writeValueAsBytes(subject);
        } catch (JsonProcessingException e) {
            logger.warn("Exception occurred while executing method base64ToObject", e);
            return defaultValue;
        }
    ...
    public static<T> T bytesToObject(byte[] subject, Class<T> klass) {
        return bytesToObjectOrDefault(subject, klass, null);
    }

    public static<T> T bytesToObjectOrDefault(byte[] subject, Class<T> klass, T defaultValue) {
        if (Objects.isNull(subject)) {
            return defaultValue;
        }
        try {
