...
import com.balazskreith.vstorage.common.InvalidConfigurationException;
...
import static com.balazskreith.vstorage.rxutils.RxBuffer.NOT_STARTED;
...
public class RxCollector<T> extends Observable<List<T>> implements Observer<T>, Consumer<List<T>> {
    ...
    private AtomicLong started = new AtomicLong(NOT_STARTED);
    ...
    private void start() {
        if (!this.started.compareAndSet(NOT_STARTED, Instant.now().toEpochMilli())) {
            return;
        }
        this.timeout.start();
    }
