...
public interface Mapper<U, R> {
    ...
    static<T, U> Mapper<T, U> createFromRxFunc(Function<T, U> mapper, Logger _logger) {
    ...
    static<T, U> Mapper<T, U> createFromRxFunc(Function<T, U> encoder) {
    ...
    static Mapper<byte[], String> createBase64Mapper() {
        ...
        return create(encoder::encodeToString, logger);
    }

    static Mapper<String, byte[]> createBytesToBase64Mapper() {
        Logger logger = loggers.get(BASE_64_MAPPER_LOGGER);
        var decoder = Base64.getDecoder();
        return create(decoder::decode, logger);
    }
