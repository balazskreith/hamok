...
import io.github.balazskreith.hamok.racoon.events.Events;
import io.github.balazskreith.hamok.racoon.events.InboundEvents;
import io.github.balazskreith.hamok.racoon.events.OutboundEvents;
...
public class Racoon implements Disposable, Closeable {

    private static final Logger logger = LoggerFactory.getLogger(Racoon.class);

    public static RacoonBuilder builder() {
        return new RacoonBuilder();
    }

    final RaftLogs logs;
    final RacoonConfig config;
    ...
    public void remoteRemotePeerId(UUID peerId) {
        this.remotePeers.detach(peerId);
    }

    void inactivatedLocalPeer() {
