...
//            logger.info("{} sending {}", this.getId(), response);
            ...
            if (request.lastLogIndex() < logs.getCommitIndex()) {
                // if the highest index of the candidate is smaller then the commit index of this,
...
//            logger.info("{} Restarted", this.getId());
            ...
            int extraDelayInMs = 0;
            if (this.raft.getLeaderId() == null) {
                // let's add some extra in case of split votes
                extraDelayInMs = random.nextInt(config.electionMaxRandomOffsetInMs());
            }
            var delayInMs = config.electionMinTimeoutInMs() + random.nextInt(config.electionMaxRandomOffsetInMs()) + extraDelayInMs;
